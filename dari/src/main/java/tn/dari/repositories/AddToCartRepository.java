...
public interface AddToCartRepository extends JpaRepository<AddtoCart,Long> {
    ...
    @Query("Select addCart  FROM AddtoCart addCart WHERE addCart.furniture.idFur= :furniture_id and addCart.user_id=:user_id")
    Optional<AddtoCart> getCartByFurnitureIdAnduserId(@Param("user_id")Long user_id,@Param("furniture_id")Long furniture_id);
    @Modifying
    @Transactional
