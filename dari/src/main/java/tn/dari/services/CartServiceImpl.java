...
import tn.dari.repositories.AddToCartRepository;
import tn.dari.repositories.CheckoutRepository;
...
public class CartServiceImpl implements CartService {
    ...
    @Autowired
    FurnitureService furServices;
    private static final Logger logger = LoggerFactory.getLogger(CartServiceImpl.class);

    @Override
    public List<AddtoCart> addCartbyUserIdAndFurnitureId(long furnitureId, long userId,int qty,double price) throws Exception {
        try {
            if(addCartRepo.getCartByFurnitureIdAnduserId(userId, furnitureId).isPresent()){
                throw new Exception("Furniture already exist.");
            }
            AddtoCart obj = new AddtoCart();
            obj.setQty(qty);
            obj.setUser_id(userId);
            Furniture fur = furServices.getFurnitureById(furnitureId);
            obj.setFurniture(fur);
    ...
    @Override
    public List<CheckoutCart> saveFurnitureForCheckout(List<CheckoutCart> tmp) throws Exception {
        try {
